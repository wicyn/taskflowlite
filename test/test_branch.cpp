...
/// @brief Branch / MultiBranch 测试 —— 条件路由、协作式多路放行。
///
/// 覆盖接口（Branch 单目标互斥）：
///   - Branch::select(index)                  按索引选择单个后继
///   - Branch::select_if(pred)                谓词选择首个匹配
///   - Branch::reset()                        清除选择（不放行任何后继）
///   - Branch::operator[](index)              下标 Proxy，赋值 bool
///   - Branch::size()                         后继数
///
/// 覆盖接口（MultiBranch 多目标累积）：
///   - MultiBranch::select(i, j, k...)        多索引同时放行
///   - MultiBranch::select_if(pred)           选所有匹配
///   - MultiBranch::select_all()              全选广播
///   - MultiBranch::reset()                   清空累积选择
///   - MultiBranch::operator[](i)             单索引 Proxy
///   - MultiBranch::operator[](i, j, ...)     多索引 Proxy（编译期等长校验）
///
/// Branch 走"协作式 (差额)"语义：未选中的后继并非永久阻塞；
/// 但本测试场景里 Branch 节点是其后继的唯一前驱，因此未选中即不会执行。
...
/// @test [branch][select] Branch::select(i) 仅放行索引 i 的后继。
TEST_CASE("Branch: select(index) 仅放行单个后继", "[branch][select]") {
    TestEnv env;
...
/// @test [branch][select_if] select_if 选首个谓词匹配的后继（短路）。
TEST_CASE("Branch: select_if 谓词选首个匹配", "[branch][select_if]") {
    TestEnv env;
    tfl::Flow flow;
...
/// @test [branch][reset] Branch::reset() 不放行任何后继。
TEST_CASE("Branch: reset 阻塞所有后继", "[branch][reset]") {
    TestEnv env;
    tfl::Flow flow;
...
TEST_CASE("Branch: operator[] 下标语法", "[branch][operator]") {
    TestEnv env;
    tfl::Flow flow;
...
/// @test [branch][multi][select] MultiBranch::select(i, j) 同时放行多个。
TEST_CASE("MultiBranch: select(...) 多索引广播", "[branch][multi][select]") {
    TestEnv env;
    tfl::Flow flow;
    std::atomic<int> ran[4] = {0, 0, 0, 0};

    auto mb = flow.emplace([](tfl::MultiBranch& m) {
        m.select(0, 2);  // 放行 0 和 2，跳过 1 和 3
    });
    auto p0 = flow.emplace([&] { ran[0].store(1); });
    auto p1 = flow.emplace([&] { ran[1].store(1); });
    auto p2 = flow.emplace([&] { ran[2].store(1); });
...
/// @test [branch][multi][select_all] select_all 广播全部。
TEST_CASE("MultiBranch: select_all 广播全部后继", "[branch][multi][select_all]") {
    TestEnv env;
    tfl::Flow flow;
...
/// @test [branch][multi][select_if] select_if 选所有匹配（与 Branch::select_if 不同）。
TEST_CASE("MultiBranch: select_if 选所有匹配", "[branch][multi][select_if]") {
    TestEnv env;
    tfl::Flow flow;
...
/// @test [branch][multi][reset] reset() 清空累积选择。
TEST_CASE("MultiBranch: reset 清空累积", "[branch][multi][reset]") {
    TestEnv env;
    tfl::Flow flow;
    std::atomic<int> hits{0};
...
TEST_CASE("MultiBranch: operator[多索引] 编译期等长校验", "[branch][multi][operator]") {
    TestEnv env;
    tfl::Flow flow;
    std::atomic<int> ran[3] = {0, 0, 0};

    auto mb = flow.emplace([](tfl::MultiBranch& m) {
        m[0, 1, 2] = {true, false, true};
    });
    auto p0 = flow.emplace([&] { ran[0].store(1); });
    auto p1 = flow.emplace([&] { ran[1].store(1); });
    auto p2 = flow.emplace([&] { ran[2].store(1); });
