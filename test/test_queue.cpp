...
/// @details 恰好推入 capacity 个元素，强制触发单元素竞争（pop vs steal 仲裁）。
///          用 CHECK + 守卫替代裸 REQUIRE，避免多线程中断言失败后继续执行。
    ...
    "[queue][bounded][stress][mt]")
{
    ...
    REQUIRE_FALSE(q.try_push(reinterpret_cast<int*>(kN + 1)));  // 满
    ...
    "[queue][unbounded][stress][mt]")
{
    ...
    auto check = [&](int* p) {
        ...
        REQUIRE(v <= kN);
        std::size_t idx = static_cast<std::size_t>(v - 1);
        REQUIRE_FALSE(seen[idx].exchange(true));
        consumed.fetch_add(1, std::memory_order_relaxed);
        };
