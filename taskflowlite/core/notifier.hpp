...
///
/// prepare_wait / commit_wait / cancel_wait 三态机 + 64 位状态字（epoch +
/// prewaiter count + 侵入式栈顶）单原子保护。notify_one / all / n 三种唤醒粒度。
/// 关键优化：_park 中 CAS 检测窗口唤醒，跳过不必要的 OS 挂起 syscall。
...
public:
    ...
    /// @brief 获取等待者池的容量（最大并发 worker 数）。
    /// @return 等待者数组的元素总数（从构造参数 n 确定，运行期不变）。
    [[nodiscard]] std::size_t size() const noexcept { return m_waiters.size(); }

    /// @brief 统计当前正处于 kWaiting OS 挂起状态的等待者数量。
    /// @return 需要逐一检查 state，此值为 O(n) 遍历快照，不保证精确。
    ...
    /// @return 2^16 - 1 = 65535，由状态字中 16-bit stack 字段决定。
