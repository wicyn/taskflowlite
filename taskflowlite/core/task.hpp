...
/// @brief 任务句柄 - DAG 节点的轻量级代理
///
/// @details
/// 内部持有指向 Work 节点的指针，用于：
/// - 建立任务间依赖关系（DAG 边）
/// - 绑定信号量约束
/// - 注册观察者
///
/// @note 所有权：弱引用，不负责内存管理（归属 Flow/Graph）
class Task {
    friend class Flow;
...
inline bool Task::operator==(const Task& rhs) const noexcept { return m_work == rhs.m_work; }
inline bool Task::operator!=(const Task& rhs) const noexcept { return m_work != rhs.m_work; }
inline void Task::reset() noexcept { m_work = nullptr; }

inline std::size_t Task::hash_value() const noexcept { return std::hash<const Work*>{}(m_work); }
inline std::string_view Task::name() const noexcept { return m_work->m_name; }
inline bool Task::valid() const noexcept { return m_work != nullptr; }
inline std::size_t Task::num_successors() const noexcept { return m_work->m_num_successors; }
inline std::size_t Task::num_predecessors() const noexcept { return m_work->_num_predecessors(); }
inline std::size_t Task::num_acquires() const noexcept { return m_work->_num_acquires(); }
inline std::size_t Task::num_releases() const noexcept { return m_work->_num_releases(); }
inline std::size_t Task::num_observers() const noexcept { return m_work->m_observers ? m_work->m_observers->observers.size() : 0; }
inline Task::operator bool() const noexcept { return m_work != nullptr; }
inline bool Task::has_exception_ptr() const noexcept { return m_work->m_exception_ptr != nullptr; }
inline std::exception_ptr Task::exception_ptr() const noexcept { return m_work->m_exception_ptr; }
inline TaskType Task::type() const noexcept { return m_work->type(); }
...
inline Task& Task::clear_predecessors() noexcept { m_work->_clear_predecessors(); return *this; }
inline Task& Task::clear_successors() noexcept { m_work->_clear_successors(); return *this; }
...
template <std::invocable<Semaphore&> F>
void Task::for_each_acquire(F&& visitor) noexcept(std::is_nothrow_invocable_v<F, Semaphore&>) {
...
template <std::invocable<Semaphore&> F>
void Task::for_each_release(F&& visitor) noexcept(std::is_nothrow_invocable_v<F, Semaphore&>) {
...
public:
    ...
    void for_each_acquire(F&& visitor) const noexcept(std::is_nothrow_invocable_v<F, const Semaphore&>);

    template <std::invocable<const Semaphore&> F>
    void for_each_release(F&& visitor) const noexcept(std::is_nothrow_invocable_v<F, const Semaphore&>);
...
inline bool TaskView::operator==(const TaskView& rhs) const noexcept { return &m_work == &rhs.m_work; }
inline bool TaskView::operator!=(const TaskView& rhs) const noexcept { return &m_work != &rhs.m_work; }
inline std::size_t TaskView::hash_value() const noexcept { return std::hash<const Work*>{}(&m_work); }
inline std::string_view TaskView::name() const noexcept { return m_work.m_name; }
inline std::size_t TaskView::num_predecessors() const noexcept { return m_work._num_predecessors(); }
inline std::size_t TaskView::num_successors() const noexcept { return m_work.m_num_successors; }
inline std::size_t TaskView::num_acquires() const noexcept { return m_work._num_acquires(); }
...
template <std::invocable<const Semaphore&> F>
void TaskView::for_each_acquire(F&& visitor) const noexcept(std::is_nothrow_invocable_v<F, const Semaphore&>) {
...
template <std::invocable<const Semaphore&> F>
void TaskView::for_each_release(F&& visitor) const noexcept(std::is_nothrow_invocable_v<F, const Semaphore&>) {
