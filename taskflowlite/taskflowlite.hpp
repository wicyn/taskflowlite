...
#define TASKFLOWLITE_VERSION_MINOR 3
...
struct Version2 {
    ...
    constexpr Version2(std::uint32_t maj, std::uint32_t min, std::uint32_t pat) noexcept
        : major{maj}, minor{min}, patch{pat} {}

    /// @brief C++20 三路比较运算符
    constexpr std::strong_ordering operator<=>(const Version2&) const = default;
    ...
    /// @brief 支持 std::ostream 流式输出版本号
    friend std::ostream& operator<<(std::ostream& stream, const Version2& ver) {
        return stream << ver.major << '.' << ver.minor << '.' << ver.patch;
    }
};
