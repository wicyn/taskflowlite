...
///   - on_after(WorkerView)  在每次任务 invoke 之后同步调用；
...
// ─────────────────────────────────────────────────────────────
// Observer 2：执行时长统计（线程局部 begin 时间戳，避免锁）
// ─────────────────────────────────────────────────────────────
class TimingObserver : public tfl::TaskObserver {
    using Clock = std::chrono::steady_clock;
    using TP    = Clock::time_point;

    // 使用 thread_local 给每个 worker 记录自己的开始时间戳
    // ——这是规避锁的关键技巧，回调跑在 worker 线程，TLS 天然分隔
    static thread_local TP s_begin;
...
thread_local TimingObserver::TP TimingObserver::s_begin{};
...
// ─────────────────────────────────────────────────────────────
int main() {
    std::cout << "=== Example 15: TaskObserver Performance Tracing ===\n\n";
    ...
    auto timing  = B.register_observer<TimingObserver>();
    auto tracer  = B.register_observer<TraceLogger>();

    // 给 A/C/D/E 也挂 timing，看哪条路径最慢
    auto t_a = A.register_observer<TimingObserver>();
    auto t_c = C.register_observer<TimingObserver>();
    auto t_d = D.register_observer<TimingObserver>();
