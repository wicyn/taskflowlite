...
///   - AsyncTask: 每个任务是独立的引用计数句柄，可在运行期通过 task.start(deps...)
///                动态拼接依赖，适合"依赖关系运行期才确定"的 ETL / pipeline 场景。
///
/// 关键 API：
///   - Executor::defer_async(callable)     创建 Idle 态任务句柄
///   - task.start()           提交任务（无依赖直接启动）
///   - task.start(deps...)  提交任务并声明依赖
...
int main() {
    ...
    //
    // 5 个独立任务，依赖关系通过 task.start(deps...) 运行期建好。
    // ======================================================================
    ...
    auto extract = executor.defer_async([&] {
    ...
    auto transform_a = executor.defer_async([&] {
    ...
    auto transform_b = executor.defer_async([&] {
        std::osyncstream(std::cout) << "[Transform-B] Field aggregation (group-by, sum)...\n";
        rows_transformed.fetch_add(rows_extracted.load() / 2);
    });
    transform_b.name("transform_b");

    auto load = executor.defer_async([&] {
        std::osyncstream(std::cout) << "[Load] Writing to target warehouse (rows="
                  << rows_transformed.load() << ")\n";
    });
    load.name("load");

    auto audit = executor.defer_async([&] {
        std::osyncstream(std::cout) << "[Audit] Audit log recording completed\n";
    });
    audit.name("audit");

    // Step 2: Declare dependencies via task.start(deps...)
    // 先启动根任务（无依赖），再启动下游（声明其依赖的上游）
    extract.start();
    transform_a.start(extract);
    transform_b.start(extract);
    load.start(transform_a, transform_b);
    audit.start(transform_a, transform_b);
    ...
    // ======================================================================
    std::osyncstream(std::cout) << "\n--- Advanced: get() rethrows exceptions ---\n";
    auto risky = executor.defer_async([] {
